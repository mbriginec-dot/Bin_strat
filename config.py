...
# --- Біржі ---
# Активна зараз тільки Coinbase. Bybit тимчасово вимкнено зі сканування
# (pairs.json досі містить його список пар — щоб повернути, достатньо додати
# "bybit" назад у цей список, без інших правок). Binance виключено раніше —
# блокує дата-центри GitHub Actions, HTTP 451, обмеження за геолокацією сервера.
EXCHANGES = ["coinbase"]
...
# За замовчуванням (для бірж на кшталт Bybit/Binance, де є 4h-свічки)
EXTRA_LEVEL_TIMEFRAMES = ["4h", "2h", "1h"]

# Перевизначення додаткових таймфреймів для конкретних бірж, чий API не підтримує
# стандартний набір (наприклад Coinbase не має 4h-гранулярності — найближчий
# доступний замінник — 6h).
EXTRA_LEVEL_TIMEFRAMES_BY_EXCHANGE = {
    "coinbase": ["6h", "2h", "1h"],
}
...
def get_extra_timeframes(exchange_id: str) -> list[str]:
    """Повертає список додаткових таймфреймів для конкретної біржі (з урахуванням
    можливих обмежень її API), або стандартний список, якщо винятку немає."""
    return EXTRA_LEVEL_TIMEFRAMES_BY_EXCHANGE.get(exchange_id, EXTRA_LEVEL_TIMEFRAMES)


# Поріг сили рівня для радару/сигналу — окремо для денного і для додаткових ТФ
# (включно з "6h" на випадок Coinbase)
MIN_LEVEL_STRENGTH = {
    "1d": 3,
    "4h": 5,
    "6h": 5,
...
# --- Поріг відправки сигналу ---
# Сигнал відправляється в Telegram, тільки якщо пройдено >= цієї кількості пунктів
# чекліста (з 8 можливих, див. checklist.py).
MIN_CHECKLIST_CONFIRMATIONS = 4
