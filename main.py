...
3. Тягне 5-хвилинні свічки -> перевіряє радар (наближення 0.5%, чисто інформаційний)
   та 3 патерни (пробій/відбій/хибний пробій) - strategy.py.
4. Якщо патерн підтвердився — рахує ATR(5хв) для стопу, збирає повний чекліст
   з 8 підтверджень (checklist.py), рахує вхід/стоп/тейк, шле фінальний сигнал.
...
def process_pair(exchange, exchange_id: str, symbol: str, bot_state: dict):
    ...
    all_levels = multi_tf_levels.gather_multi_tf_levels(exchange, symbol)
    ...
    # --- 1. Радар (інформаційний, 0.5%) ---
    radar_candidates = strategy.find_radar_candidates(current_price, all_levels)
